...
| TextMesh Pro essential resources and shaders | `Assets/TextMesh Pro/**` | The resources were copied through the Unity editor, but the repository does not contain a Unity/TMP license file or evidence tying the copied files to an exact source package/version. The separately listed font and EmojiOne files do not resolve the remaining resources. | **Not approved for release.** In the pinned editor, identify the exact source package/version and applicable license/notice, archive that evidence, and record its redistribution requirements here before RC. |
