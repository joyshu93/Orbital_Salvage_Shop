...
`GameApp.Workbench.cs` presents the object, visible targets, findings and tool tray using uGUI/TMP. Tapping a named target button or numbered marker always inspects and focuses it. Selecting a tool preserves the observation; **Use here** executes the focused combination. `WorkbenchToolDrag` also supports deliberate drag/drop and cancels a drag when its page is disabled. A drop updates the focused target so a subsequent button action cannot silently use an older target. `WorkbenchToolIcon` draws tool silhouettes; `WorkbenchAtmosphere` animates decorative reactions without delaying domain completion. `Localizer` selects English/Korean; workbench content uses `LocalizedCopy`, including retained messages, so returning after a language change displays the selected language. `GameApp.Awake` enables `TMP_Settings.useModernHangulLineBreakingRules` before creating UI, so Korean text uses word boundaries for wrapping instead of routinely splitting a word between syllables.

`WorkbenchCaseContext` supplies immutable bilingual requests and stage-specific previous-discovery recaps. `GameApp.WorkbenchContext.cs` keeps the request and current question visible, renders named target controls and explicit tool selection, and derives the notebook from known observations. Updated observations use completed-action results and related-target changes; the notebook shares that lookup. The notebook temporarily disables underlying buttons and restores their previous interactability and selection when closed. Target/tool/notebook state is transient and adds no save fields. Ordinary Selectable tint is neutral so pointer focus cannot darken semantic selection colors. Ice04 has two actions instead of repeating Ice02's three-action sequence; Ice01 sealing requires the crack inspection and drying but no unrelated leaf-reading gate. Both cases retain the same ten stage IDs and now contain 27 actions.
...
`BuildAll` also configures the imported workbench atlases as Sprite textures with NPOT scaling disabled and full-rectangle sprite meshes. The existing `workbench-states.png` keeps its 2048 maximum size; the supplementary `watch-discovery-states.png` uses 4096 to preserve its 2172-pixel width. `ProjectBuilder.ConfigureWorkbenchAtlas` sets the default platform limit as well as the importer limit, and `VisualAssetImporter` recognizes the watch atlas's 4096 limit during reimport. Run `BuildAll` after content, localization, scene, font, icon, player-setting or art changes; change catalogs/builders instead of hand-editing generated assets.
...
`Assets/Resources/Art/Workbench/watch-discovery-states.png` supplements that atlas with three equal horizontal panels in a 2172 × 724 image: the discovered watch with its leaf caught inside, the opened glass cover with the leaf still caught, and the opened watch after the leaf is freed. `WorkbenchArtwork` crops these panels using thirds of the actual imported width and caches the resulting sprites. Ice02 (`ice-02-spread`) completion and Ice03 (`ice-03-tomorrow`) entry use the same first panel. Ice03 switches to the second panel immediately after `clear-hinge` and to the third after `free-leaf`, so the visible state follows the relevant action rather than waiting for stage completion. The existing atlas continues to supply the other investigation states. The supplementary image also uses an opaque dark-plum backdrop; its provenance and visual limitations remain documented with the art records.
